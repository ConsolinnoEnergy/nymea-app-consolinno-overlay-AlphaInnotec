...
function exportImage() {
  ...
  if [ ${scaleFactor} -ne 1 ]; then
    ...
    $INKSCAPE_BIN AppIcon.svg -o AppIcon.iconset/icon_${size}x${size}@${scaleFactor}x.png -C -w $finalSize -h $finalSize
  else
    $INKSCAPE_BIN AppIcon.svg -o AppIcon.iconset/icon_${size}x${size}.png -C -w ${size} -h ${size}
  fi
}
...
iconutil -c icns AppIcons.iconset
